...
async function assign(teacherId: number) {
  ...
  try {
    ...
    const emailNote = result.emails?.enabled
      ...
      : ''
    const calendarNote = result.calendarSync ? ' Calendars updated for student and teacher.' : ''
    successMessage.value =
      (result.message ||
...
// A request opened from Announcements lands here with the store already primed.
onMounted(() => {
      ...
      <div class="review-main">
        ...
        <div v-else class="review">
          <div class="student-block">
            ...
            <p v-if="selected.preferredSubjects.length" class="preference-note">
              Detected preference:
          ...
          <div v-if="selected.confirmedSchedule" class="confirmed-schedule">
            ...
            <p>
              <strong>{{ selected.confirmedSchedule.title }}</strong>
            </p>
            <p>
          ...
          <div v-if="selected.request.status === 'pending'" class="assignment">
            ...
            <p class="hint assign-hint">
              One teacher covers the full block ({{ formatRequestWindow(selected.request.slots) }},
              {{ requestDurationMinutes(selected.request.slots) }} minutes). Ranked by availability,
              workload, subject expertise, and remarks.
            </p>
                ...
                <div class="candidate-top">
                  <div>
                    ...
                    <p class="muted">
                      ...
                      · workload {{ teacher.workload }} · score {{ teacher.suitabilityScore }}
                    </p>
                  </div>
                  <button
                    type="button"
              ...
              <ul v-if="slot.availableTeachers.length" class="teacher-list">
                <li v-for="teacher in slot.availableTeachers" :key="teacher.id">
                  <span>
                    {{ teacher.fullName }}
                    <span class="muted">
                      · {{ teacher.subjectExpertise || 'General' }} · load
                      {{ teacher.workload ?? 0 }}
                    </span>
                  </span>
                </li>
              ...
              <template v-if="slot.unavailableTeachers.length">
                ...
                <ul class="teacher-list conflicts">
                  <li v-for="teacher in slot.unavailableTeachers" :key="teacher.id">
                    ...
                    <span class="conflict"> Busy: {{ teacher.conflict.title }} </span>
                  </li>
                </ul>
              </template>
            </div>
...
.queue-count {
  color: var(--lh-warm);
  border: 1px solid color-mix(in srgb, var(--lh-warm) 28%, transparent);
...
.class-block {
  ...
  border-radius: 0.65rem;
  border: 1px solid color-mix(in srgb, var(--lh-accent) 30%, transparent);
  background: var(--lh-accent-soft);
  color: var(--lh-ink);
  font-size: 0.9rem;
  line-height: 1.45;
